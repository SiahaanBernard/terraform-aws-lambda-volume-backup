...
class SnapshotManagement(object):
    ...
    def __get_snapshots_by_state(snapshots, state):
        return list(filter(lambda x: x["State"] == state, snapshots))
    ...
    def __get_snapshots_by_backup_tag(snapshots, backup_tag):
        return [
            x
            for x in filter(
                lambda x: "BackupTag" in [y["Key"] for y in x["Tags"]],
                filter(lambda x: "Tags" in x, snapshots)
            )
            for y in x["Tags"]
    ...
    def __get_oldest_snapshot_ids(snapshots, number_of_snapshots):
        snapshots = map(lambda x: x["SnapshotId"], sorted(
    ...
    def execute(self):
        ...
        for volume_id, snapshots in self.snapshots.items():
